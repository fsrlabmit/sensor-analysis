# # Update Rationale:
# In response to a high observed rate of recombination in previous analyses,
# this update aims to improve barcode specificity and reduce false-positive matches.
# This will be accomplished by extending the barcode length to include an additional 8 nucleotides
# of the sensor sequence.
# Update: Kexin Dong, Sep 7, 2025
#
# Update 2: Kexin Dong, 2026-05-21
# Added region-based QC strategies (port of sensor_extraction_42nt_trimming_quality.py).
# A new positional arg `quality_check_mode` controls which QC mode is applied.
...
if len(sys.argv) != 12:
    print("Usage: python3 sensor_extraction_peg_counts_ext_bc_16nt.py <input_df> <R1_FILE> <R2_FILE> splitby proto_mismatches_allowed bc_len real_bc_len sensor_len quality_check_mode -o <folder_name>")
    sys.exit(1)
...
real_bc_len = int(sys.argv[7])
sensor_len = int(sys.argv[8])
quality_check_mode = str(sys.argv[9])
folder_name = str(sys.argv[11])
...
def quality_checker(q1, q2, bc_len, sensor_len, proto_len=19,
                    mode='region_average',
                    min_q=MIN_QUALITY,
                    max_lowq_bases=MAX_LOWQ_BASES,
                    min_q_per_base=MIN_QUALITY_PER_BASE):
    """
    Check read quality using different QC strategies.

    For the prime-editing extended-BC layout, R1 = [16 nt BC] + [55 nt sensor],
    where the last 8 nt of the BC region overlap with the first 8 nt of the sensor
    (real_bc_len=8). Region-based QC checks [:bc_len + sensor_len] which covers
    the entire used portion of R1.
    ...
    bc_len : int
        Barcode length matched against bc_list (16 in PE layout).
    sensor_len : int
        Sensor length downstream of the BC start (55 in PE layout).
    proto_len : int
        Protospacer length in R2.
    mode : str
        QC mode. One of:
        - 'full_average'                  : original SG behavior, full-read mean
        - 'region_average'                : mean over used region only
        - 'region_average_and_threshold'  : region mean + per-base BC check + per-base lowQ count
    """
...
def to_IOSeq_rec(sensor_seq, i, q1, real_bc_len, sensor_len):
    """ 
    Modified to only extract 42 nt sensor
    After 15 nt barcode
    """
    qname = 'read_' + str(i)
    record = SeqRecord(Bio.Seq.Seq(sensor_seq[real_bc_len:real_bc_len+sensor_len]), id=qname, name=qname, description='', dbxrefs=[])

    #add quality score to the record
    qual = (np.frombuffer(q1, dtype=np.uint8) - 33)
    record.letter_annotations["phred_quality"] = qual[real_bc_len:real_bc_len+sensor_len]
    
    return record

def extraction_filtration(folder_name, input_df,R1_FILE,R2_FILE, bc_len, sensor_len, splitby, proto_mismatches_allowed, quality_check_mode='region_average', breakpoint=False):
    ...
    os.makedirs(folder_name, exist_ok=True)#make the new directory (needs to be a non-existent folder)
    ...
    d1 = pd.DataFrame(dict(zip(['Guide_ID', 'sgRNA_no_Gstart', 'unique_BC'], [list(input_df['pegRNA_id']), proto_list, bc_list])))
    ...
    for i, ((proto1, r1, q1), (proto2, r2, q2)) in enumerate(zip(fastq_reader(R1_FILE, gz=GZ), fastq_reader(R2_FILE, gz=GZ)), 1):
        ...
        quality_out = quality_checker(q1, q2, bc_len, sensor_len, mode=quality_check_mode)
...
#---RUNNING THE SCRIPT------
count_df, class_df, mat = extraction_filtration(folder_name, input_df,R1_FILE,R2_FILE, bc_len, sensor_len, splitby, proto_mismatches_allowed, quality_check_mode=quality_check_mode, breakpoint=False)

#save the counts and classification dataframes
folder_name_short = folder_name.split('/')[-1]
count_df.to_csv('./extended_bc_16nt_analysis/counts/' + folder_name_short + '_count_df.csv')
class_df.to_csv('./extended_bc_16nt_analysis/classification/' + folder_name_short + '_classification_df.csv')

confusion_mat = pd.DataFrame(mat)
confusion_mat.to_csv('./extended_bc_16nt_analysis/confusion_mats/' + folder_name_short + '_confusion_matrix.csv')